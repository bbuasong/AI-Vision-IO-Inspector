...
| 좌표 이미지 파일명 | `{품번}_coordinate.png` 예: `01100-51430_coordinate.png` |
| 같은 ViewType 재등록 | OldVer 백업을 만들지 않고 기존 현재 파일을 새 파일로 교체 |
| 조회 방식 | 저장된 `FilePath`로 직접 접근하며 폴더 전체 검색을 전제로 하지 않음 |
| 화면 표시 관리경로 | 실제 절대경로 대신 `REFERENCE:\\{분류코드}\{품번}` 형식으로 축약 표시 |
| 부품 삭제 | 부품 기준정보만 삭제하며 기준 이미지 파일은 프로그램이 자동 삭제하지 않음 |
| 검사 이력 | 부품 삭제와 무관하게 검사 이력은 보존 기간/디스크 여유공간 정책으로만 삭제 |

기준 이미지 파일 삭제는 다음 경우에만 허용한다.

- 단일품목 등록 화면에서 작업자가 기준 이미지 삭제 버튼을 누른 경우
- 현재6개저장 또는 검사 UI 기준 이미지 저장으로 같은 위치 이미지를 갱신하는 경우. 이때 기존 현재 파일을 백업 없이 교체한다.
- 작업자가 폴더 관리로 직접 삭제하는 경우

그 외 부품 삭제, 다중품목 CSV DB 교체, DB 재저장 동작은 기준 이미지 실제 파일을 삭제하지 않는다.
임시 이미지나 coordinate 파일을 삭제해 `DB\Image\Temp\품번`이 비면 품번 폴더를 삭제하고, 다른 품번 작업 폴더도 없으면 `Temp` 폴더까지 삭제한다.
부품등록의 `이미지 모두 삭제`는 선택 이미지 한 장이 아니라 해당 품번의 기준 이미지 6장과 coordinate 이미지, 측정부 DB 정보를 함께 삭제한다. 삭제 전 사용자 확인을 필수로 한다.
...
| --- | --- | --- | --- |
| PartNo | 품번 | Y |  |
| IndexNo | 측정부 번호 | Y | 1~5 |
| ItemType | 측정부 항목 | Y | 미설정/길이/너비/높이/두께 |
| ViewType | 기준 이미지 방향 | Y | 현재 Thickness 고정 |
| X1/Y1/X2/Y2 | 측정 위치 좌표 | N | Thickness 원본 이미지 픽셀 좌표 |
| LineColor | 표시 선 색상 | Y | `#RRGGBB` |
| NominalValue | 기준값 | Y |  |
...
| 구분 | PartType | N |  |
| 측정부1항목 / 측정부1기준 / 측정부1허용 | 첫 번째 독립 측정부 정보 | Y(헤더) | 값이 모두 비어 있거나 `-`이면 미사용 |
| 측정부1색상 | 첫 번째 측정부 표시 색상 | N | 미입력 시 빨강 |
| 측정부1X1 / 측정부1Y1 / 측정부1X2 / 측정부1Y2 | 첫 번째 측정부 좌표 | N | 좌표를 사용할 때는 네 값을 모두 입력 |
| 측정부2~측정부5의 항목/기준/허용/색상/X1/Y1/X2/Y2 | 추가 독립 측정부 | N | 중간 번호가 비어 있으면 저장 시 연속 IndexNo로 재정렬 |
| 단위 | 전체 측정부 단위 | Y | `mm`만 허용 |
