...
| O-044 | 확인필요·최우선 | `dimensions`와 `measuredValue`의 단위가 확정되지 않았습니다. 2026-09-03 AI 담당 메일의 예시는 `width 216.82`, `height 147.43`, `measuredValue 216.82`인데, 첨부 `HD_Proc.cpp`의 `HD_Get_Width`가 `cv::minAreaRect`의 `rotRect.size.width`를 그대로 넘기므로 **픽셀 값으로 보입니다**. 우리 `MeasurementService`는 `NominalValue ± Tolerance`로 비교하므로(예시 기준 149.5~150.5), 픽셀이면 측정부 판정이 항상 FAIL입니다. | 단위가 픽셀인지 mm인지, mm이면 환산 계수(캘리브레이션)가 어디서 오는지, 픽셀이면 환산을 SDK가 할지 앱이 할지 AI 담당 확인 | 측정부 판정이 실제 치수 기준으로 동작하고 결과 이미지·이력에 단위가 함께 남음 |
