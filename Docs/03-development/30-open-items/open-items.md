...
| O-042 | 원인확인·AI담당전달완료 | **SDK 가 결과 버퍼를 아예 채우지 않고 돌려주는 경우가 있습니다.** 2026-09-04 로그(`vlad-hd-json-20260904-144649.log`)에서 `[HD_INFERENCE_REQUEST]` 와 `[HD_INFERENCE_RESULT]` 의 JSON 이 **글자 하나까지 같고 Length 도 203 으로 같습니다.** `viewJudge`·`score`·`dimensions` 가 모두 앱이 넣어 보낸 초기값 0 그대로입니다. 최근 로그의 `"score"` 214건이 전부 `0.00` 입니다. 앞서 「`scoreThreshold`>0 이면 항상 0」·「SDK 가 score 를 0~1 로 다룬다」고 적었던 것은 **틀렸습니다.** DB 전수에서 threshold `39.83` 인 133건 중 127건이 score `51.95`~`100.0` 으로 정상 반환됐고, 값의 범위도 0~1 이 아닙니다(다만 앱이 `InspectionScoreFormat.Normalize` 로 0~1 이면 100 을 곱하므로 DB 값만으로 원본 척도는 단정할 수 없고, 로그에 0 아닌 값이 남은 것이 없어 로그로도 확정하지 못했습니다). **파생 문제**: 앱은 AI 판정을 그대로 따르는데(`JudgmentService.Judge` 의 `HasAuthoritativeJudgment` 경로), 갱신되지 않은 `viewJudge` 0 이 합격으로 읽혀 **검사가 실제로 수행되지 않았는데 PASS 가 납니다.** DB 에서 threshold 95 인 14건이 모두 score 0 이면서 PASS 입니다. | AI 담당에게 전달 완료(2026-09-07 사용자 확인). SDK 가 결과를 채우지 않는 조건 규명 대기. 앱에서 「요청과 결과가 동일하면 검사 실패로 본다」는 방어를 넣을지 결정 필요 | 결과 JSON 이 요청과 달라지고, 검사하지 않은 건이 PASS 로 남지 않음 |
