...
| O-042 | 원인확인·벤더대기 | `scoreThreshold`가 0보다 크면 SDK가 `score`를 항상 0으로 돌려줍니다. 2026-08-24 로그에서 threshold `0.00` 호출 16건은 실제 score(0.23~0.998), `75.00` 호출 54건은 전부 0.0이었고 요청 JSON의 다른 필드는 동일했습니다. DB 271건 전수에서 `threshold>0 이면서 score>0`인 기록은 0건입니다. 원인은 SDK가 `score`를 사양(0~100)과 달리 0~1로 다루기 때문으로 보입니다. | `Q-041` 회신으로 사양 개정 방향 확정 후 앱 반영 여부 결정 | 현장 임계값(95)에서 실제 score가 반환되고 판정이 정상 동작 |
