...
| --- | --- | --- | --- | --- |
| O-037 | 보안-조치필요 | `CFG\Config_LineBackup.json`에 현장 NVR RTSP 계정정보가 평문으로 들어 있고, 2026-08-04 커밋 `65d1fb2`부터 GitHub에 올라가 있습니다. `.gitignore`의 `CFG/*Backup*.json` 규칙이 있으나 파일명이 `Bakcup` 오타여서 걸러지지 않았습니다. | 저장소 공개 여부 확인, NVR 계정 비밀번호 변경 여부 결정, 파일을 Git에서 제외할지 결정 | 현장 계정정보가 저장소 최신 상태에 없고, 노출된 계정의 처리 방침이 정해짐 |
| O-038 | 정책/절차필요 | 설정이 `Config.json`(개발용)과 `Config_LineBackup.json`(현장용)으로 이원화되어 해상도와 RTSP 주소, 저장 경로가 다릅니다. 앱은 `Config.json`만 읽습니다. | 현장 배포 시 어느 파일을 기준으로 어떻게 반영할지 절차 확정 | 배포 절차 문서에 설정 반영 단계가 명시되고 현장 값과 일치 |
| O-039 | 확인필요 | 현장 설정에서 `CAM5`(Thickness)만 `CAM_RTSP_PREVIEW_IP`가 `streamID=2`가 아니라 `streamID=1`입니다. 다른 5채널은 검사용 `streamID=1`과 미리보기용 `streamID=2`로 분리되어 있습니다. | 의도된 설정인지, 오기인지 현장 확인 | Thickness 미리보기 스트림 기준이 확정되고 설정에 반영 |
| O-040 | 문서반영완료-검증필요 | 문서의 카메라 해상도가 개발용 `Config.json` 기준이라 현장 값과 6채널 모두 달랐습니다. 2026-08-11에 현장 기준(Top/Thickness 2448x2048, 나머지 2592x1944)으로 정정했습니다. | 실제 현장 카메라에서 해상도를 재확인 | 문서 해상도와 실제 스트림 해상도가 일치 |
| O-041 | 구현완료·DLL대기 | 학습 중에도 검사를 SDK까지 넘기고 요청 JSON에 `trainingRunning`(0/1)을 실어 보내도록 2026-08-24에 구현했습니다. 학습 중 검사 차단 2군데를 해제했습니다. 앱은 학습 여부로 캡처·파싱·판정·이력을 전혀 다르게 하지 않고 값만 실어 보냅니다(`ADR-007`). SDK의 간이 CV 검사는 아직 없습니다. | 간이 검사가 들어간 DLL로 교체 후 실기 검증, `Q-039`·`Q-040` 회신 반영 | 학습 중 검사가 ERROR가 아니라 평상시와 같은 흐름으로 판정되고, 로그 요청에 `"trainingRunning":1`이 확인됨 |
| O-042 | 원인확인·AI담당전달완료 | **SDK 가 결과 버퍼를 아예 채우지 않고 돌려주는 경우가 있습니다.** 2026-09-04 로그(`vlad-hd-json-20260904-144649.log`)에서 `[HD_INFERENCE_REQUEST]` 와 `[HD_INFERENCE_RESULT]` 의 JSON 이 **글자 하나까지 같고 Length 도 203 으로 같습니다.** `viewJudge`·`score`·`dimensions` 가 모두 앱이 넣어 보낸 초기값 0 그대로입니다. 최근 로그의 `"score"` 214건이 전부 `0.00` 입니다. 앞서 「`scoreThreshold`>0 이면 항상 0」·「SDK 가 score 를 0~1 로 다룬다」고 적었던 것은 **틀렸습니다.** DB 전수에서 threshold `39.83` 인 133건 중 127건이 score `51.95`~`100.0` 으로 정상 반환됐고, 값의 범위도 0~1 이 아닙니다(다만 앱이 `InspectionScoreFormat.Normalize` 로 0~1 이면 100 을 곱하므로 DB 값만으로 원본 척도는 단정할 수 없고, 로그에 0 아닌 값이 남은 것이 없어 로그로도 확정하지 못했습니다). **파생 문제**: 앱은 AI 판정을 그대로 따르는데(`JudgmentService.Judge` 의 `HasAuthoritativeJudgment` 경로), 갱신되지 않은 `viewJudge` 0 이 합격으로 읽혀 **검사가 실제로 수행되지 않았는데 PASS 가 납니다.** DB 에서 threshold 95 인 14건이 모두 score 0 이면서 PASS 입니다. | AI 담당에게 전달 완료(2026-09-07 사용자 확인). SDK 가 결과를 채우지 않는 조건 규명 대기. **앱 쪽 방어는 넣지 않기로 했습니다**(`ADR-008`, 2026-09-07). 판정 주체는 AI 이고, 앱이 결과를 되짚어 판정을 뒤집으면 규칙이 두 곳으로 갈라지기 때문입니다. 그동안은 이력의 PASS 중 `Score` 가 `0.00` 인 줄은 검사가 돌지 않았을 수 있다고 보아야 합니다 | 결과 JSON 이 요청과 달라지고, 검사하지 않은 건이 PASS 로 남지 않음 |
| O-043 | 확인필요 | 2026-08-24 `rtsp-frame-metrics` 로그에서 6채널 모두 실제 수신 해상도가 1920x1080으로, 설정값(Top/Thickness 2448x2048, 나머지 2592x1944)과 다릅니다. 학습은 원해상도로 하고 추론은 1920x1080으로 하는 상태일 수 있습니다. | NVR 스트림 설정과 카메라 설정 확인, 추론 입력 해상도가 학습과 같아야 하는지 AI 담당 확인 | 수신 해상도와 설정/학습 기준이 일치하거나, 달라도 되는 근거가 확정 |
| O-044 | 단위확정·실측검증필요 | **`dimensions`와 `measuredValue`의 단위는 mm 로 확정됐습니다(2026-09-03 사용자 확인).** 처음에는 첨부 `HD_Proc.cpp`의 `HD_Get_Width`가 `cv::minAreaRect`의 `rotRect.size.width`를 그대로 넘기는 것만 보고 픽셀로 추정했으나, 그 함수를 호출하는 쪽이 첨부에 없어 환산 지점을 확인하지 못한 상태였습니다. 2026-09-03 AI 담당 메일 예시의 `measuredValue 216.82` 와 `nominalValue 150.0` 이 어긋나 보인 것은, 그 요청의 `measurementPoints` 가 사양서·벤더 샘플(`Form1.cs`)의 보일러플레이트 값(`nominalValue 150.00`, `x1 120.50`, `y1 240.00`, `x2 360.50`, `y2 240.00`)을 그대로 쓴 시험 호출이었기 때문입니다. | 실제 치수를 아는 부품으로 측정값과 실측을 대조. 카메라·거리별로 환산이 맞는지 Top 과 Thickness 각각 확인 | 실측과 `measuredValue` 가 허용 범위 안에서 일치하고, 측정부 판정이 실제 치수 기준으로 동작 |
| O-045 | 확인필요 | 측정부 방향이 구분되지 않습니다. 첨부 `HD_Proc.cpp`의 `HD_Get_Width`는 `*org_val = w`로 **항상 width**를 `measuredValue`에 넣고, 앱이 보낸 `measurementPoints`의 좌표(`x1,y1,x2,y2`)는 사용하지 않습니다. AI 담당도 "높이는 Width가 아닌 Height를 사용하면 됩니다"라고 안내했습니다. | 좌우 길이 측정부와 상하 높이 측정부를 SDK가 어떻게 구분하는지, 앱이 방향 정보를 추가로 보내야 하는지 확인 | 같은 부품에 길이·높이 측정부를 함께 등록해도 각각 맞는 값이 반환됨 |
| O-046 | 원인확인·SDK대기 | **학습 후 VLAD 재초기화를 하면 카메라 프레임 수신이 1/4 로 떨어지고 회복하지 않습니다.** 2026-09-04 현장 로그(`D:\2026-09-04\Backup`) 세션 `112326` 에서 확인했습니다. 13:53:19~21 재초기화를 기준으로 `mon0` 수신이 재초기화 전 898구간 평균 48.3장/10초에서 후 71구간 평균 12.0장/10초로 떨어졌고 12분간 회복하지 않았습니다. 메모리는 15,113MB 에서 17,000~18,000MB 로 2.3GB 늘어난 채 유지됩니다. 콜백 스레드는 #6 에서 #41 로 바뀌어 재등록 자체는 됐습니다. 앱은 `VladCamModeRuntime.Reload()` 에서 `VLAD_Unregistration` 을 호출했고 타임아웃·access violation 로그가 없으므로 SDK 가 성공을 반환했습니다(실패였다면 예외로 재초기화가 중단됩니다). 그런데도 메모리와 수신량이 돌아오지 않는 것으로 보아, **SDK 가 해제 성공을 반환하면서도 이전 세션의 RTSP 클라이언트·스레드를 정리하지 않는 것으로 보입니다.** 두 세션이 같은 NVR 스트림을 동시에 당겨 경합하는 상태로 추정합니다(이 부분은 추론입니다). 학습 중에는 스트리밍이 정상이며, 문제는 학습이 끝난 뒤 재초기화 시점에만 생깁니다. 부수 문제로 `RestoreCameraStreamsAfterTraining()` 의 호출부가 `HandleTrainingExited` 한 곳뿐이라 Exited 가 오지 않으면 복구가 아예 실행되지 않고, 앱에 학습 중지·취소 경로가 없어 프로그램 재시작 말고는 되돌릴 방법이 없습니다. | AI 담당에게 `VLAD_Unregistration` 이후 이전 세션의 RTSP 클라이언트·스레드·메모리가 실제로 해제되는지 확인 요청. 해제가 불완전하면 재초기화 대신 프로그램 재시작을 안내할지 결정 | 학습 후 프레임 수신이 학습 전 수준(4.9fps)으로 돌아오고 메모리가 회복됨 |
| O-047 | 구현완료·AI회신대기 | 측정부 항목에 내경·외경을 추가했습니다. 항목은 `MeasurementItemType` enum(0 미설정 ~ 6 외경)과 `MeasurementItemTypePolicy`(이름·코드값·그리는 모양)로 모았습니다. enum 값이 그대로 요청 JSON 의 `itemType` 으로 나갑니다. 위치지정 팝업 왼쪽에 항목 선택을 두고, 선 항목은 두 점 찍기, 내경·외경은 눌러서 끄는 사각 드래그로 갈라 그립니다. 사각은 좌상단·우하단으로 정렬해 저장하고 화면에는 네모와 그 안의 내접원을 함께 그립니다. DB 는 예전처럼 한글 이름을 문자열로 저장하므로 스키마 변경이 없습니다. **2026-09-07**: 회신을 기다리지 않고 보낸 사양대로 마무리했습니다(사용자 결정). 남아 있던 `InspectionRowViewModel.GetMeasurementDimensionName` 의 내경·외경 매핑을 넣었습니다. 그전에는 이름이 「…- 내경」인 측정부가 어느 자리에도 들어가지 못해, 길이 측정부가 하나라도 있으면 **내경·외경 값이 통째로 버려졌습니다.** 표시는 앞 네 자리(길이/너비/높이/두께)를 그대로 두고, 내경·외경은 쓰는 부품에서만 이름을 붙여 뒤에 덧붙입니다(`10 / 20 / 30 / - / 내경 5 / 외경 6`). 여섯 자리로 고정하면 이 둘을 쓰지 않는 대부분의 부품에서 「- / -」만 길어집니다. 6가지로 확인했습니다. | 실기 검증만 남았습니다. 내경·외경 측정부를 등록해 검사하고 값이 오는지 확인 | 내경·외경 측정부를 등록해 검사하면 값이 반환되고 판정·이력·결과 이미지에 반영됨 |
| O-048 | 구현완료·현장검증필요 | 이력의 중복 표시를 정리했습니다. `MismatchItems` 를 없애고 `NgResult` 하나로 합쳤으며, NG 사유에서 측정값·기준값 반복과 결과 메시지 전체 복사를 제거해 원인만 남겼습니다(측정부 이름 / Score 미달 방향 / 검사 실패). 메시지 칸에서 눈으로 찾아야 했던 점수는 `Score` 칸을 새로 만들어 뺐습니다. DB 는 방향별 판정·점수·치수를 `History_CapturedImages` 에 `view_judge`·`score`·`dimension_width`·`dimension_height`·`dimension_depth` 로 담도록 넓혔습니다(schema_version 3). 기존 1,587행을 그대로 두고 새 열은 NULL 로 두는 것을 DB 사본으로 확인했습니다. | 현장에서 이력 화면과 CSV 를 확인. `result_message` 에서 방향별 점수 나열을 뺄지는 방향별 열이 쌓인 뒤 결정 | 같은 값이 한 칸에만 나오고, 방향별 점수를 이력에서 조회할 수 있음 |
| O-049 | 구현완료·AI회신대기 | 부품 기준정보에 「품목개수」(`PartCount`, 1~255, 기본 1)를 넣었습니다. 오링처럼 여러 개가 한 세트인 부품에서 개수가 맞는지 AI 가 보게 하려는 것입니다. **개수를 세고 판정하는 일은 SDK 가 하고, 앱은 기준값만 넘깁니다.** 요청 JSON 의 `partNo` 바로 다음에 `partCount` 로 실립니다. DB 는 `PartList_Parts.part_count` 와 `History_Inspections.part_count` 를 더해 schema_version 4 로 올렸고, 사본으로 부품 11,424행·이력 405행이 모두 기본값 1 로 채워지는 것과 3·255·7 의 저장·읽기 왕복을 확인했습니다. 화면은 부품 등록 Information Input(품목개수·분류코드 한 줄 반반), PART INFORMATION, DB 조회·부품 등록·다중품목 결과·이력 그리드, 이력 검색 조건에 넣었습니다. 1~255 를 벗어난 입력은 화면과 모델 양쪽에서 1 로 되돌립니다. | AI 담당 회신(`partCount` 로 개수 판정 가능 여부, 어긋날 때 `viewJudge` 불합격 처리 여부) 확인 후 실기 검증 | 세 개짜리 부품에 두 개만 올렸을 때 검사가 불합격으로 나오고, 이력에 그때의 기준 개수가 남음 |
...
| --- | --- | --- | --- |
| O-027 (구) | ~~구현 필요~~ **완료** | **2026-09-03 코드 확인 결과 이미 구현돼 있습니다.** `WpfInspectionMeasurementImageService` 299~315행이 결과 이미지에 `W / D / H`를 그리고, `InspectionWorkflowService` 576~578행이 방향별 결과에서 값을 채웁니다. 화면 표시는 `MainWindowViewModel` 4132~4149행입니다. 값이 0으로 보였던 것은 SDK가 `dimensions`를 0.0으로 반환했기 때문이며, 2026-09-03 AI 담당 수정으로 실제 값이 들어오기 시작하면 앱 수정 없이 표시됩니다. 남은 것은 저장뿐입니다(`O-029` 신규). | 정정 완료. 이 번호는 재번호 이전 ID라 현행 `O-027`과 다릅니다 |
| O-028 (구) | ~~구현 필요~~ **완료** | **2026-09-03 코드 확인 결과 이미 구현돼 있습니다.** `WpfInspectionMeasurementImageService` 437~439행이 결과 이미지에 `측정 {값} 기준 {값} (-{하한} ~ +{상한})`을 그리고, 428행이 측정부 번호를 좌표 위에 라벨로 찍습니다. 측정값은 `History_Measurements.measured_value`로 DB에도 저장됩니다. | 정정 완료. 이 번호는 재번호 이전 ID라 현행 `O-028`과 다릅니다 |
...
| O-029 | 부분완료 | W/D/H는 검사 화면과 **결과 이미지까지 전달·표시됩니다**(2026-09-03 코드 확인). 저장만 빠져 있습니다 — `History_Inspections`에 W/D/H 컬럼이 없습니다. 측정값은 `History_Measurements.measured_value`로 이미 저장됩니다. 2026-09-03 AI 담당 수정으로 `dimensions`에 실제 값이 들어오기 시작하므로(이전에는 전부 0.0) 보존 여부를 정해야 합니다. | W/D/H 이력 보존 필요 여부 확정 후 DB 마이그레이션 결정. 단위(픽셀/mm)가 `O-044`에서 확정된 뒤 저장해야 의미 있는 값이 남습니다 | 재실행 후 이력에서도 W/D/H 조회 가능하거나 미보존 정책 확정 |
