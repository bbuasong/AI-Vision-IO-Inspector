...
- 현재 소스(`Codes\AI-Vision IO Inspector`)를 기준으로 프로그램 구조, 검사 흐름, VLAD MAT JSON, OCR 미등록 품번 등록 전환, RTSP/카메라, Thickness coordinate 이미지, 데이터 저장/이력 구조를 다시 분석했습니다.
- 최신 분석 결과를 `Docs\03-development\current-program-status-2026-08-10.md`로 작성했습니다. 이번 작업은 문서 갱신만 수행했고 코드/XAML/설정 파일은 수정하지 않았습니다.
- `VLAD_HD_Inference_Mat`는 별도 Result 조회가 아니라 8192-byte UTF-8 공용 JSON 버퍼를 in-place로 갱신하는 현재 계약으로 반영되어 있음을 확인했습니다. 요청 JSON에는 `viewJudge`, `score`, `dimensions`, `measurements` 결과 자리가 포함되어 있습니다.
- `VLAD_Custom_Registration`의 `modelPath`는 `CFG\Config.json`의 `MODEL=RuntimeData/Models/VLAD/Ex_Weight`를 EXE 폴더 기준 절대경로로 해석하고, 호출 직전 trailing slash를 붙여 전달하는 구조임을 확인했습니다. 2026-08-10 실행 로그 기준 실제 전달값은 `C:\LinkGenesis\AI-Vision IO Inspector\Run_2608010b\RuntimeData\Models\VLAD\Ex_Weight\`입니다.
- OCR 미등록 품번 흐름은 확인 팝업 후 단일품목 등록 화면으로 이동하고, OCR 품번이 `RegistrationPartNo`에 자동 입력되는 구조로 반영되어 있음을 확인했습니다.
- `open-items.md`를 2026-08-10 기준으로 갱신해 RTSP 단독 로그 검증, 고장 카메라 포함 검사 정책, Thickness UI/coordinate 기준 이미지 검증, 과거 Result/Data 용어 정리, 프로그램 아이콘 적용 여부를 잔여 항목으로 분리했습니다.
- 실제 장비/VLAD DLL 호출과 전체 솔루션 빌드는 이번 문서 작업 범위에서 실행하지 않았습니다.

## 2026-08-11

- AI 어시스턴트 협업 방식으로 구현과 검증을 서로 다른 모델에 분담하는 안을 검토했습니다. 정적 리뷰만으로는 네이티브 버퍼 소유권, P/Invoke 마샬링, RTSP 콜백 스레드 문제를 판별할 수 없으므로, 검증의 1차 게이트는 빌드/테스트/실기 구동에 두고 다른 모델은 요구사항 정합성과 논리 결함을 보는 2차 감사 역할로 배치하는 방향으로 정리했습니다. 현재 단위 테스트 프로젝트와 CI 워크플로가 없다는 점을 선행 과제로 확인했습니다.
- 프로그램 실제 위치를 `Codes\AI-Vision IO Inspector`로 확인했습니다. 루트 `README.md`가 존재하지 않는 `Tests\AI-Vision IO Inspector` 경로를 기준으로 작성되어 있어 기재된 빌드 명령 두 줄이 그대로 실패하는 상태였습니다.
- 루트 `README.md`를 현재 소스 기준으로 다시 작성했습니다. 솔루션 경로를 `Codes\AI-Vision IO Inspector\AI.Vision.IOInspector.sln`로 정정하고, 프로젝트 목록을 `App`/`Application`/`Domain`/`Infrastructure`/`Vision` 5개로 수정했습니다. `VisionWorker`는 2026-08-05 커밋 `e09cc04`에서 제거된 사실을 명시했습니다.
- 루트 `README.md`에 현재 기준값을 반영했습니다. Assembly Version 1.1.0.0, `win-x64`, `SelfContained=false`, Release `DebugType=none`, 검사 통과 점수 95.00, 유사도 기준 99.00입니다.
- `CFG\Config.json`의 `CUSTOM.HD.CAMS`를 직접 확인해 카메라 구성표를 작성했습니다. `CAM0` Top 2592x1944, `CAM1~CAM4` Front/Back/Left/Right 1920x1080, `CAM5` Thickness 2592x1944이며 6채널 모두 `CAM_TYPE=RTSP`, `CAM_ENABLED=true`, 30 FPS입니다.
- 앱 시작 흐름을 `AppBootstrapper` 기준으로 갱신했습니다. 기존 문서의 `RuntimeAssemblyResolver` 표기를 실제 구현인 `NativeDependencyLoader`로 정정하고, `VisionRuntimeFactory.BeginInitializeVladRuntimeOnStartup()` 백그라운드 예약 구조를 반영했습니다.
- 검사 흐름과 VLAD MAT JSON 계약 절을 현재 기준으로 다시 작성했습니다. 과거 `VLAD_Inference_Mat`/`VLAD_Custom_InferenceData_V1` 표기를 제거하고 `VLAD_HD_Inference_Mat` 3인자 in-place JSON 방식, 8192-byte UTF-8 고정 버퍼, `scoreThreshold` 단일 판정 기준으로 정리했습니다.
- `Docs\03-development\README.md`를 갱신했습니다. 목록에서 빠져 있던 `current-program-status-2026-08-10.md`, `project-structure-*.md`, `ocr-sample-api-integration-2026-07-23.md`, `score-slider-update-2026-07-16.md`, `work-log-2026-07-15.md`, `work-log-automation.md`, `vision\` 폴더를 추가하고 문서를 현재 기준/누적 기록/규칙/시점 스냅샷/기능별 상세로 분류했습니다.
- Git 작업 트리 상태를 확인했습니다. 현재 개발 소스인 `Codes\AI-Vision IO Inspector\`가 untracked이고, `Codes\Version1_0_0_0`, `Codes\Version1_1_0_0`, `Tests\`, `Docs\05-simulator`가 삭제 상태로 남아 총 1,838개 삭제와 73개 untracked 항목이 있습니다. 폴더 재구성이 Git에 반영되지 않은 상태이므로 루트 `README.md`에 주의 항목으로 남기고 이번 작업에서는 Git 조작을 하지 않았습니다.
- `task-board.md`를 2026-05-31 기준에서 2026-08-11 기준으로 갱신했습니다. `T-006` AI 연동, `T-007` 카메라 SDK, `T-008` 품번 입력 방식, `T-160` 실제 카메라 연결, `T-170` 연속 영상 미리보기, `T-190` VLAD 추론 연결을 현재 구현 상태로 정정하고, `T-180` 카메라 보정은 앱이 단위 변환을 하지 않고 AI 반환 mm 값을 그대로 사용하는 현재 방침에 맞춰 `미적용-AI담당`으로 변경했습니다.
- `task-board.md`의 `O-` 참조가 현재 `open-items.md`와 어긋나 있던 부분을 정정했습니다. 기존 `T-008`은 `O-009`(History 보존 정책)를 가리키고 있었으나 실제 관련 항목은 `O-015`(Scanner OCR)입니다. `다음 처리 순서`도 2026-08-10 현장 검증 잔여 항목 기준으로 다시 작성했습니다.
- `task-board.md`에 최근 구현 항목 `T-200` 기준 이미지 병합, `T-210` 유사도 검색, `T-220` OCR 미등록 품번 등록 전환, `T-230` 이력 보존/삭제 정책을 추가했습니다.
- `questions.md`를 2026-08-11 기준으로 갱신했습니다. 그 사이 확정된 `Q-003` 카메라 연동 방식, `Q-020` AI 호출 방식, `Q-023` 카메라 SDK, `Q-025` 품번 입력 방식을 `완료`로 변경하고, 미확정 정책인 `Q-037` 고장 카메라 검사 정책과 `Q-038` 결과 이미지 W/D/H·측정값 합성을 추가했습니다.
- `questions.md`의 `O-` 참조가 `open-items.md` 재번호 이후 어긋나 있던 8건을 정정하고 정정 내역표를 문서 하단에 남겼습니다. `Q-025`는 `O-009`(History 보존 정책)를 가리키고 있었으나 실제로는 `O-015`(Scanner OCR)이며, `Q-032`/`Q-033`/`Q-034`/`Q-035`도 한 칸씩 밀린 참조였습니다.
- `questions.md`의 `우선 답변 필요 항목`을 현재 기준으로 다시 작성했습니다. 해소된 카메라 SDK·바코드 장비 항목을 빼고 새 export 포함 DLL 제공 시점, 최종 추론 모델 구조, pixel-to-mm 담당 범위, 고장 카메라 정책, 보존 정책, CUDA 런타임 설치 기준으로 정리했습니다.
- `roadmap.md`에 2026-08-11 진행 상태표를 추가했습니다. Phase 0~3은 완료, Phase 4는 결과 이미지 합성 미구현으로 부분완료, Phase 5는 DLL 교체와 6채널 실기 검증 대기로 정리했습니다.
- `Docs\03-development`를 읽는 시점 기준 세 폴더로 재편했습니다. `10-before-development`는 작업 시작 전에 읽는 현황과 경위, `20-during-development`는 구현 중 지켜야 할 규칙과 확정 결정 및 VLAD SDK 계약, `30-open-items`는 잔건과 미확정 질문입니다.
- `10-before-development`로 `current-program-status-2026-08-10.md`, `project-structure-2026-06-16/19/22.md`, `version-1.1.0.0-merge-2026-07-24.md`, `version-management-2026-07-24.md`, `roadmap.md`, `work-log.md`, `work-log-2026-07-15.md`, `changelog.md`, `ocr-sample-api-integration-2026-07-23.md`, `score-slider-update-2026-07-16.md` 12개를 옮겼습니다.
- `20-during-development`로 `coding-rules.md`, `checklist.md`, `review-checklist.md`, `codex-task-template.md`, `work-log-automation.md`, `decisions.md`와 `vision\` 폴더(29개 파일)를 옮겼습니다.
- `30-open-items`로 `open-items.md`, `questions.md`, `task-board.md`를 옮겼습니다.
- 세 폴더에 각각 `README.md`를 작성했습니다. `10`은 먼저 읽을 문서와 시점별 스냅샷 주의사항, `20`은 규칙·확정 결정·VLAD 계약의 현행/이전 구분과 구현 시 주의 지점 5개, `30`은 잔건을 DLL 대기·실기 검증·AI 담당자 확인·정책 결정·배포 환경 5개 범주로 묶은 내용입니다.
- `Docs\03-development\README.md`를 상위 안내로 다시 작성하고 재편 전후 경로 대조표 22행을 넣었습니다.
...
- `Docs\07-AI IO Inspector Manual(Wpf)\AI-Vision_IO_Inspector_사용자매뉴얼_v0.2_20260903.docx`와 같은 이름의 PDF를 만들었습니다. 36페이지입니다.
- v0.1(2026-08-12) 이후 커밋 46건 중 사용자 화면에 드러나는 변경을 반영했습니다.
- 화면 캡처 11장을 `D:\2026-08-31\Backup`의 2026-08-31 빌드 촬영본으로 전면 교체했습니다. v0.1은 2026-08-06~07 개발 환경 촬영본이었습니다.
- 신설한 절
  - **4.3 측정부는 이렇게 판단합니다** — 판정 대상 측정부(좌표 + 기준값 둘 다 있음)와 참고값 측정부의 구분, 참고값의 기준값이 AI 값으로 표시되는 이유, 허용 범위 읽는 법, Thickness 칸 판정 숨김 규칙.
  - **8. 편하게 쓰는 법** — 자주 쓰는 흐름 3가지, 시간을 줄이는 습관 6가지, 헷갈리기 쉬운 곳 6가지.
  - **부록 B** — v0.1 대비 변경점 15건과 참고 절.
- 개편한 절
  - 5.1.4 측정부 등록 — 카메라별 관리(Top/Thickness 각 최대 5개)로 전면 개편.
  - 5.1.3 기준 이미지 등록 — 벌(Set) 개념 반영.
  - 3.3.3 Front 기준 이미지 칸 신설, 3.2.2 「구분」→「메모」 정정.
- 본문에 적은 규칙은 커밋 메시지가 아니라 코드로 확인했습니다. `MeasurementService.CompareMeasurements`(판정 조건), `MeasurementRegion.HasDrawnCoordinates`, `MeasurementPointPolicy`(Top/Thickness, 최대 5개), `MainWindowViewModel.ShouldShowJudgment`(Thickness 칸 판정 숨김)입니다.
- 작성 환경에 pandoc과 Node가 없어 `python-docx`로 생성하고, Word COM으로 목차를 갱신한 뒤 PDF로 내보냈습니다. PyMuPDF로 페이지를 렌더링해 표·그림·강조 상자 배치를 눈으로 확인했습니다.
- **현장 검증은 하지 않았습니다.** 캡처는 2026-08-31 빌드 기준이므로, 이후 화면이 또 바뀌었으면 해당 그림을 교체해야 합니다.

### 측정 단위 mm 확정 (`O-044`)

- 사용자 확인으로 `dimensions`와 `measuredValue`의 단위가 **mm** 로 확정됐습니다.
- 앞서 픽셀로 추정했던 것은 첨부 `HD_Proc.cpp`의 `HD_Get_Width`가 `cv::minAreaRect` 결과를 그대로 넘기는 부분만 보고 판단한 것이었습니다. 그 함수를 부르는 쪽이 첨부에 없어 환산 지점을 확인하지 못한 상태였습니다.
- AI 담당 메일 예시에서 `measuredValue 216.82`와 `nominalValue 150.0`이 어긋나 보였던 것은, 그 요청의 `measurementPoints`가 사양서·벤더 샘플(`Form1.cs`)의 보일러플레이트 값을 그대로 쓴 시험 호출이었기 때문입니다. 실제 부품 기준값이 아니었습니다.
- 매뉴얼 v0.2 네 곳(용어의 측정부·W/H/D, 3.2.3 MEASUREMENT, 4.2 결과 표, 4.3.3 허용 범위)에 mm 를 명시하고 문서를 다시 생성했습니다. 36페이지는 그대로입니다.
- `O-044`는 「단위확정·실측검증필요」로 상태를 바꿨습니다. 남은 것은 실제 치수를 아는 부품으로 측정값과 실측을 대조하는 일입니다.

## 2026-09-07

### 품목개수(`partCount`) 추가 (`O-049`)

- 오링처럼 여러 개가 한 세트인 부품에서 개수가 맞는지 보려는 것입니다. 세 개여야 하는데 두 개만 올라온 것을 잡습니다.
- **개수를 세고 판정하는 일은 SDK 가 합니다.** 앱은 부품 기준정보에 등록해 둔 기준 개수를 실어 보내기만 합니다. 사용자 표현으로 "참고정보를 넘긴다"입니다.
- 범위는 1~255, 기본 1 입니다. 벗어난 값은 **잘라내지 않고 1 로 되돌립니다.** 255 로 자르면 사용자가 넣으려던 값과도 다르고 화면에 뜬 값과도 달라집니다. 규칙을 `Part.PartCount` setter 한 곳에 두어 화면·CSV 어느 쪽에서 들어와도 같게 다룹니다.
- 요청 JSON 은 `partNo` 바로 다음에 `partCount` 를 넣습니다. 버퍼 증가는 약 17 byte 이고, 요청이 200 byte 안팎이라 8192 byte 한도에 영향이 없습니다.
- 화면 입력칸은 **칸을 떠날 때**(LostFocus) 값을 살핍니다. 글자를 칠 때마다 살피면 "1"을 지우는 순간 빈 값이 되어 1 이 도로 들어차고, 그러면 값을 고쳐 쓸 수 없습니다.

#### DB

- `schema_version` 3 → 4. `PartList_Parts.part_count`, `History_Inspections.part_count` 를 `INTEGER NOT NULL DEFAULT 1` 로 더했습니다.
- 실제 DB 사본으로 확인했습니다. 앱과 같은 빌드로 마이그레이션 코드를 그대로 돌렸습니다(118 ms).

| 확인 항목                     | 결과                  |
|-------------------------------|-----------------------|
| `schema_version`              | 3 → 4                 |
| `PartList_Parts` 11,424행     | 전부 `part_count = 1` |
| `History_Inspections` 405행   | 전부 `part_count = 1` |
| 부품 3 저장 후 읽기           | 3                     |
| 부품 255(경계값) 저장 후 읽기 | 255                   |
| 부품 300 / 0 입력             | 모델에서 1            |
| 이력 7 저장 후 읽기           | 7                     |

#### 화면

- 부품 등록 Information Input — 품목개수와 분류코드를 한 줄에 반씩 나눴습니다(사용자 목업).
- PART INFORMATION — 줄 수를 늘리지 않으려고 같은 방식으로 품목개수·분류코드를 한 줄에 두었습니다.
- DB 조회, 부품 등록, 다중품목 결과, 이력 표에 「품목개수」 열을 넣었습니다. 이력 검색 조건에도 넣었습니다.
- 폰트와 스타일은 기존 것을 그대로 씁니다(`FieldLabel`, `DarkTextBox`, `InfoLabelCell`, `InfoValueCell`).

#### CSV

- 부품·이력 모두 품명 다음 자리에 「품목개수」를 넣었습니다.
- 읽을 때는 「품목개수」/`PartCount`/`Part Count`/`Count` 를 받고, 열이 없으면 1 로 둡니다. 옛 파일이 그대로 읽힙니다.

**실기 검증은 하지 않았습니다.** AI 담당 회신(개수 판정 가능 여부, 어긋날 때 `viewJudge` 처리)이 남아 있습니다.

### 화면 열 이름과 CSV 머리글 맞추기

사용자가 이력의 「NG 사유」가 CSV 에서는 「NG결과」로 나오는 것을 지적해 나머지도 함께 살폈습니다.

| 위치                  | 화면    | CSV(이전) | 조치                                   |
|-----------------------|---------|-----------|----------------------------------------|
| 이력 NG 사유          | NG 사유 | NG결과    | CSV 를 「NG 사유」로                   |
| 이력 검색 라벨        | NG결과  | —         | 라벨을 「NG 사유」로                   |
| 이력 Score            | Score   | **없음**  | CSV 에 「Score」 열 추가               |
| 부품 측정부 기준값    | 기준값  | …기준     | CSV 를 「…기준값」으로(옛 이름도 읽음) |
| 검사 화면 MEASUREMENT | 기준    | —         | 화면을 「기준값」으로                  |

- 이력 CSV 의 Score 누락은 2026-09-04 에 화면에만 넣고 CSV 에 넣지 않은 것입니다. 「결과」 다음 자리에 채웠습니다.
- 측정부 CSV 머리글은 읽는 쪽에 옛 이름 폴백을 두었습니다. `GetCsvValue` 가 정확 일치로 찾으므로 「…기준값」과 「…기준」이 섞여도 서로 걸리지 않습니다.

### 이력 표 빈 칸 열

- 「메시지」 칸 뒤에 폭 60 짜리 빈 열을 넣었습니다. 마지막 칸은 오른쪽 경계선이 없어 폭을 늘릴 수 없었습니다.
- 사용자가 통계 PASS 표처럼 해 달라고 했는데, **그 표에는 빈 열이 따로 있는 것이 아니었습니다.** 열 너비 합이 화면보다 좁아 오른쪽이 비어 보이는 것이었습니다. 이력 표는 가로가 화면을 넘으므로 같은 방식이 되지 않아 빈 열을 실제로 넣었습니다.

### 실행해서 확인한 것

앱을 띄워 실제로 눌러 보며 확인했습니다. 검사용으로 잠시 관리자 권한 요구를 낮춰 빌드했고, 확인이 끝난 뒤 되돌려 다시 빌드했습니다. 데이터 루트가 실제 프로젝트 폴더로 잡히지 않도록 실행 폴더에 빈 솔루션 파일을 두고 돌렸습니다.

| 확인한 것              | 방법                                            | 결과                                        |
|------------------------|-------------------------------------------------|---------------------------------------------|
| 화면 연결(바인딩) 오류 | 탭 11개를 모두 열고 WPF 추적 로그를 파일로 받음 | 0건                                         |
| 품목개수 입력          | 칸에 값을 넣고 다른 칸으로 옮김                 | 3·7·12·255 그대로, 300·256·0·-5·abc 는 1 로 |
| 이력 거르기            | 품목개수 칸에 「2」 입력                        | 405건 → 0건                                 |
| 이력 표 빈 칸 열       | 표를 오른쪽 끝까지 밀어 화면 확인               | 메시지 칸 뒤에 붙음                         |
| CSV 칸 수              | 머리글 수와 값 수 비교                          | 부품·이력 모두 일치                         |
| CSV 왕복               | 내보낸 값을 그대로 다시 읽음                    | 품목개수 3, 측정부 기준값 150.25 유지       |
| 옛 CSV 읽기            | 품목개수 열 없고 「…기준」인 파일               | 읽힘, 품목개수는 1                          |
| CSV 의 잘못된 값       | 300 / 0 / 글자                                  | 모두 1, 255 는 그대로                       |
| 마이그레이션 반복      | 같은 DB 에 세 번                                | 결과 같음                                   |

확인 중에 **다중품목 등록 화면의 CSV 안내 문구가 실제와 달랐던 것**을 찾아 고쳤습니다. 품목개수가 빠져 있었고 측정부 열 이름이 옛 「…기준」이었습니다.

앞서 한 번은 품목개수 칸에 「3」을 넣었는데 1 로 돌아온 것처럼 보였습니다. 칸에 포커스를 주지 않고 값만 밀어 넣어 「칸을 떠날 때」 규칙이 돌지 않은 것이었고, 칸을 눌러 포커스를 준 뒤 다시 해 보니 3 이 그대로 남습니다. 코드 문제가 아닙니다.

**하지 못한 것**: AI 호출로 `partCount` 가 실제 요청에 실려 나가는지는 SDK 를 태워야 알 수 있어 확인하지 못했습니다. 현장에서 검사 로그의 요청 JSON 을 보고 확인해야 합니다.

### 화면 손질 세 가지 (사용자 요청)

**1. 검사 화면 버튼이 한가운데 떠 보이던 것**

「기준 이미지 저장 / 검사 시작 / 화면 초기화」가 칸 한가운데에 있어 위아래로 빈 자리가 컸습니다. 이 칸은 오른쪽 「Front 기준 이미지」와 같은 줄에 있어 그 이미지 높이만큼 늘어나고, 버튼은 높이가 36 으로 고정이라 늘어난 자리의 한가운데에 놓였습니다. 칸(`Border`)을 위로 붙여 내용 높이만 쓰게 했습니다. 버튼과 상태·결과 글이 위에 나란히 붙고 빈 자리가 사라졌습니다.

**2. 품목개수 이름표에 범위 표시**

「품목개수」 → 「품목개수 (1~255)」. 요청은 (1~256) 이었지만 앞서 정한 범위가 1~255 이고 모델·DB·CSV 가 모두 그 기준이라 255 로 적었습니다.

**3. 이력의 품목개수 찾기를 같은 값으로**

다른 칸처럼 글자가 들어 있는지로 찾고 있어 「1」을 넣으면 1, 10, 11, 100 이 모두 걸렸습니다. 개수는 숫자라 그럴 이유가 없어 같은 값만 보도록 `MatchesPartCount` 를 따로 두었습니다.

지금 이력이 모두 1 이라 화면만으로는 예전 방식과 구별되지 않아, 거르는 판단을 직접 불러 15가지로 확인했습니다.
...
| 255                      | 255                | 걸림        |
| ` 3 ` (앞뒤 공백)        | 3                  | 걸림        |
| 빈 칸 / 공백만 / 값 없음 | 7                  | 거르지 않음 |
| abc                      | 1                  | 안 걸림     |
| 0                        | 1                  | 안 걸림     |

화면에서도 「11」·「2」·「0」·「x」는 0건, 「1」과 빈 칸은 405건으로 나옵니다.

### score 가 0 으로 남던 원인 재조사 — 앞선 결론을 정정 (`O-042`, `Q-041`)

사용자가 「AI 에서 제대로 작업을 안 해서 Score 를 0 으로 줬던 것 같다」고 해서 다시 봤습니다. **사용자 판단이 맞고 앞서 적은 결론이 틀렸습니다.**

**결정적 증거** — `DB\Logs\20260904\vlad-hd-json-20260904-144649.log`

```
[HD_INFERENCE_REQUEST] Length=203
{"partNo":"01100-51430","viewName":1,...,"viewJudge":0,"score":0.00,"scoreThreshold":95.00,...}
[HD_INFERENCE_RESULT]  Length=203
{"partNo":"01100-51430","viewName":1,...,"viewJudge":0,"score":0.00,"scoreThreshold":95.00,...}
```

요청과 결과가 **글자 하나까지 같고 Length 도 같습니다.** SDK 가 `viewJudge`·`score`·`dimensions` 를 하나도 채우지 않고 버퍼를 그대로 돌려준 것입니다. 최근 로그에 남은 `"score"` 214건이 전부 `0.00` 입니다.

**앞서 틀리게 적은 것**

| 앞서 적은 것                            | 실제                                                                                                                                                                                                            |
|-----------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `scoreThreshold`>0 이면 score 가 항상 0 | 아닙니다. threshold `39.83` 인 133건 중 127건이 score `51.95`~`100.0`                                                                                                                                           |
| SDK 가 score 를 0~1 로 다룬다           | DB 값 범위는 `22.92`~`100.0` 입니다. 다만 앱이 0~1 이면 100 을 곱하므로(`InspectionScoreFormat.Normalize`) DB 만으로 원본 척도는 단정할 수 없고, 로그에 0 아닌 값이 남은 것이 없어 로그로도 확정하지 못했습니다 |

앞선 조사는 threshold 값과 시기가 함께 바뀐 구간만 보고 threshold 탓으로 단정한 것이었습니다. 같은 날(08-24) 안에서도 threshold `0.0`·`39.83` 은 값이 나오고 `70`·`75` 는 0 이라 threshold 영향처럼 보였지만, 날짜를 넓혀 보면 08-25~08-28 에는 계속 정상이었고 08-23·09-01·09-04 만 전부 0 입니다.

**함께 찾은 파생 문제 — 검사하지 않은 건이 PASS 로 남습니다**

앱은 AI 판정을 그대로 따릅니다(`JudgmentService.Judge` 의 `HasAuthoritativeJudgment` 경로). 설계대로입니다. 그런데 SDK 가 버퍼를 안 채우면 앱이 넣어 보낸 `viewJudge` 0 이 그대로 돌아오고, 이것이 **합격**으로 읽힙니다. score 기준(95)은 이 경로에서 아예 보지 않습니다.

DB 에서 threshold 95 인 14건이 **모두 score 0 이면서 PASS** 입니다. 검사가 실제로 수행되지 않았는데 합격으로 남은 것입니다.

AI 담당에게는 이미 전달된 사안입니다(2026-09-07 사용자 확인). 앱에서 「요청과 결과가 같으면 검사 실패로 본다」는 방어를 넣을지는 결정이 필요합니다.

### 내경·외경 마무리 (`O-047`)

회신을 기다리지 않고 보낸 사양대로 마무리했습니다(사용자 결정).

`InspectionRowViewModel.GetMeasurementDimensionName` 에 내경·외경이 없어 이름이 「…- 내경」인 측정부는 어느 자리에도 들어가지 못했습니다. 그리고 측정부 벌이 하나라도 잡히면 나머지(`fallbackValues`)는 버리므로, **길이 측정부가 하나라도 있으면 내경·외경 값이 통째로 사라졌습니다.**

표시는 앞 네 자리(길이/너비/높이/두께)를 그대로 두고, 내경·외경은 쓰는 부품에서만 이름을 붙여 뒤에 덧붙입니다.

```
10 / 20 / 30 / -            (내경·외경을 쓰지 않는 부품)
10 / 20 / 30 / - / 내경 5    (내경을 쓰는 부품)
```

여섯 자리로 고정하면 이 둘을 쓰지 않는 대부분의 부품에서 「- / -」만 길어집니다. 이름이 붙어 있으므로 자리를 세지 않아도 됩니다.

확인 중에 측정부 이름 형식이 「Thk 1 - 길이」·「Top 3 - 길이」인 것을 DB 로 확인했습니다. 처음에는 「Thickness 측정부1 - 내경」으로 시험해 내경이 두께 자리로 들어갔는데, 실제 이름에는 `Thickness` 라는 낱말이 없어 그런 일이 생기지 않습니다. 다만 카메라 이름이 항목 이름과 같은 낱말이 되면 오분류될 여지는 남아 있습니다.

### 판정 방어를 넣지 않기로 함 (`ADR-008`)

SDK 가 결과를 채우지 않으면 검사하지 않은 건이 PASS 로 남는 문제에 대해, 앱에서 요청 JSON 과 결과 JSON 을 견줘 같으면 검사 실패로 돌리는 방어를 넣을지 물었습니다.

**넣지 않기로 했습니다.** 판정은 AI 판정을 그대로 따릅니다.

이유는 판정 주체가 AI 이기 때문입니다. 앱이 결과를 되짚어 판정을 뒤집기 시작하면 판정 규칙이 앱과 AI 두 곳으로 갈라집니다. 결과를 채우지 않는 것은 SDK 쪽에서 고칠 일이고 AI 담당에게 이미 전달된 사안입니다.

고쳐지기 전까지는 **이력의 PASS 중 `Score` 가 `0.00` 인 줄은 검사가 돌지 않았을 수 있다고 보아야 합니다.** 앱은 이것을 걸러내지 않습니다. 이 위험을 `ADR-008` 과 `O-042` 에 적어 두었습니다.
