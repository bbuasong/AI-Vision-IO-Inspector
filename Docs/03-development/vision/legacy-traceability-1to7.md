...
현재 방향은 새 구조를 유지하되, 기존 담당자가 아는 핵심 함수명을 `VLAD_Ops_Ai`, `VLAD_Ops_RTSP`, `VLAD_Ops_imvCam`에 직접 남겨서 기존 코드와 현재 코드가 서로 추적되도록 하는 것입니다.
...
| 5 | AI/VLAD 등록과 추론 | `VLAD_Ops_Ai_Env_Start`, `VLAD_Registration`, `VLAD_Ops_Inference_Registration`, `VLAD_Inference_Mat` 순서 확인 가능 | `VLAD_Ops_Ai`, `VladSdkSession`, `VladVisionInferenceEngine` | 실제 VLAD_SDK.dll x64 호출과 모델 경로 검증 |
| 6 | AI 결과 파싱 | `VLAD_InferenceData_V1_Draw`, `VLAD_InferenceData_V2_Draw`, `VLAD_InferenceData_Get_Valid_Count`, class string/color 함수 확인 가능 | `VLAD_Ops_Ai`, `VladInferenceResultParser` | detect data를 `VisionInspectionOutput.Measurements`로 변환 |
...
| 기존 담당자가 찾을 이름 | 현재 파일 |
| --- | --- |
| `VLAD_Ops_Ai_Env_Start` | `LegacyVlad/VLAD_Ops_Ai.cs`, `LegacyVlad/VladSdkSession.cs` |
| `VLAD_Registration` | `LegacyVlad/VLAD_Ops_Ai.cs`, `LegacyVlad/VladNativeMethods.cs` |
| `VLAD_Ops_Inference_Registration` | `LegacyVlad/VLAD_Ops_Ai.cs`, `LegacyVlad/VladNativeMethods.cs` |
| `VLAD_Inference_Mat` | `LegacyVlad/VLAD_Ops_Ai.cs`, `Engines/VladVisionInferenceEngine.cs` |
| `VLAD_InferenceData_V1_Draw` | `LegacyVlad/VLAD_Ops_Ai.cs`, `LegacyVlad/VladInferenceResultParser.cs` |
| `VLAD_InferenceData_V2_Draw` | `LegacyVlad/VLAD_Ops_Ai.cs`, `LegacyVlad/VladInferenceResultParser.cs` |
...
- 실제 카메라 SDK 호출은 `ImvCameraDevice`, 실제 VLAD 호출은 `VLAD_Ops_Ai`와 `VladNativeMethods` 안에 둡니다.
