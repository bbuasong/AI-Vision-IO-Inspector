...
현재 요구사항의 최종 판정에는 측정부별 측정값, 기준값, 허용값, 단위, OK/NG가 필요하다. 2026-07-07 기준 운영 방향은 AI가 측정값까지 반환하는 방식이며, 아래 두 방식 중 첫 번째를 현재 계약으로 본다.
...
### 2026-07-07 결과 문자열 계약

현재 AI 결과 반환 형식은 다음 문자열 계약으로 정리한다.
...
- 첫 번째 값은 이미지 AI 검사 OK/PASS 여부다.
- 두 번째 값은 AI score이며, `98`처럼 0~100 점수로 오면 내부 confidence는 `0.98`로 정규화한다.
- 세 번째 값부터는 현재 부품의 측정부 `IndexNo` 오름차순에 대응하는 실제 측정값이다.
- 측정값 단위는 현재 프로젝트 기준인 `mm`로 고정한다.
- 현재 애플리케이션 비교 로직은 `MeasurementRegion.Id`를 기준으로 동작하므로, 파서에서 `IndexNo` 순서값을 DB ID로 변환해야 한다.
- 이 문자열 계약은 `detectText` 또는 AI DLL 반환 문자열에서 파싱한다. 기존 TLV/bbox 파싱은 보조 또는 호환 경로로 남긴다.
...
| AI 출력 | 결과 문자열은 `isMatched,score,measurement1...N` 형식으로 정리됨. score 범위, 값 개수 불일치 처리, 실제 DLL 반환 위치는 구현 검증 필요 |
