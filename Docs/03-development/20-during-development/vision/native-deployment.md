...
## VladRuntimeSettings.json 기준

`CFG\VladRuntimeSettings.json`은 VLAD 카메라/모델 설정과 분리해서 런타임 파일 경로만 관리합니다.
...
- `VladSdkDllPath`는 `VLAD_SDK.dll` 파일 경로입니다. 상대 경로이면 실행/프로젝트 루트 기준으로 해석하고, 해당 폴더를 `SetDllDirectory`로 등록합니다.
- C# `DllImport`는 런타임 문자열을 직접 받을 수 없으므로 P/Invoke 선언은 `VLAD_SDK.dll` 이름을 유지하고, DLL 탐색 폴더만 이 설정으로 제어합니다.
- `StudyDirectoryPath`, `StudyBatchFilePath`는 이미지 학습 시작 기능에서 실행할 `Study.bat` 위치입니다.
