...
| V-008 | 결과 파싱 | 규격확정-구현필요 | AI 결과 문자열은 `true/false,score,measurement1...N` 형식으로 정리됨. `VladInferenceResultParser` 또는 별도 parser 구현 필요 |
| V-009 | 측정값 변환 | 부분완료 | 현재 `VladMeasurementMapper`는 키워드/bbox 기반. 새 문자열 결과는 `IndexNo` 순서값을 `MeasurementRegion.Id`로 변환해야 함 |
...
| QV-002 | AI 결과 문자열이 실제 DLL의 어느 반환값에 들어오는가? | 계약은 `true/false,score,measurement1...N`으로 정리됐지만, 실제 `detectText`/TLV/별도 export 중 어느 경로인지 구현 확인이 필요합니다. |
...
## 2026-07-07 AI 결과 문자열 체크

- 예시 결과 `true,98,100,159,25,47`은 `IsMatched=true`, `Confidence=0.98`, 측정부1=100mm, 측정부2=159mm, 측정부3=25mm, 측정부4=47mm로 해석한다.
- 현재 결과 모델은 이 값을 담을 수 있지만, 실제 파서와 `IndexNo -> MeasurementRegion.Id` 매핑 구현은 남아 있다.
- 측정부 값 개수가 DB 측정부 개수와 다르면 부족분은 `AI 측정값 없음`, 초과분은 로그 후 무시로 처리한다.
