# Development

개발 진행에 필요한 로드맵, 작업 보드, 규칙, 체크리스트, 로그, 질문, 의사결정을 관리합니다.

| 파일 | 내용 |
| --- | --- |
| `roadmap.md` | 투입 초기부터 구현/검증까지의 작업 순서 |
| `task-board.md` | 현재 작업 목록과 완료 기준 |
| `coding-rules.md` | C# 코딩 규칙과 주석 원칙 |
| `checklist.md` | 투입 초기, 구현 전, 구현 후 체크리스트 |
| `review-checklist.md` | 코드 리뷰 및 자체 점검 항목 |
| `codex-task-template.md` | Codex 작업 요청 템플릿 |
| `changelog.md` | 의미 있는 변경 이력 |
| `questions.md` | 확인이 필요한 질문 |
| `decisions.md` | 확정된 의사결정 기록 |
| `work-log.md` | 날짜별 작업 기록 |
