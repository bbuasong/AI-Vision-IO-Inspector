...
namespace AI.Vision.IOInspector.Infrastructure.Services
{
    ...
    public static class EpsonScanClient
    {
                                                  ...
                                                  double minConfidence = 0.80)
        {
            ...
            // ocrOnly=true : 서버가 PDF(iText) 생성을 건너뛰고 OCR+part_no만 함(32비트 메모리 절약, DPI 유지).
            string url = baseUrl.TrimEnd('/') + "/scan-to-pdf?ocrOnly=true";

            // ScanToPdfRequest 스키마. 익명타입은 var 로 유지해야 직렬화가 비지 않습니다.
            //  card = null : 라벨 자동 크롭 건너뜀(32비트 메모리 부족 회피). 반드시 명시적 null 이어야 서버가 끔.
            var payload = new
            {
