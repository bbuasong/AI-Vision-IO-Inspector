...
public class JobRegistry
{
    ...
    private async Task SaveAsync()
    {
        ...
        try
        {
            // jobs.json은 API가 재시작된 뒤에도 이어서 처리해야 하는 작업만 보관합니다.
            // WPF가 마지막 작업을 삭제한 뒤에는 빈 배열 파일을 남기지 않아 다음 스캔에서
            // 이전 작업이 남아 있는 것처럼 보이지 않도록 파일 자체를 제거합니다.
            if (_jobs.IsEmpty)
            {
            ...
            await File.WriteAllTextAsync(_persistPath, JsonSerializer.Serialize(_jobs.Values.ToList(), Jso));
        }
        catch { }
