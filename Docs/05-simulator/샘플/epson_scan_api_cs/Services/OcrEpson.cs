...
public sealed class OcrEpson : IDisposable
{
    ...
    private static Dictionary<string, object> StructureToFields(List<Letter> letters)
    {
        ...
        // 부품번호: 첫 줄에서 괄호류 앞까지, 단 부품번호다운 토큰만 결합 (한글/잡음 '미' 등 제외)
        const string Brackets = "()[]{}<>«»‹›＜＞｛｝〈〉《》";
        string partNo = "", partNoSub = "";
        if (lines.Count > 0)
        {
            var pn = new System.Text.StringBuilder();
            foreach (var w in lines[0])
            {
                if (w.Text.Any(ch => Brackets.Contains(ch))) { partNoSub = w.Text; break; }
                if (LooksLikePartNo(w.Text)) pn.Append(w.Text);
            }
            partNo = pn.ToString();
        }
    ...
    private static PdfFont LoadKoreanFont()
    {
        ...
        foreach (var f in candidates)
        {
            ...
            try
            {
                _cachedFont = PdfFontFactory.CreateFont(f,
    ...
    // 부품번호 토큰 판정 (Python _looks_like_partno와 동일): 영숫자(A-Z,0-9) + 하이픈류만, 영숫자 최소 1개.
    //   숫자/영문/조합 모두 허용 → '미'·도장 '검수' 같은 한글, 점 든 "s.p.a", 순수 기호/하이픈은 제외.
    private static bool LooksLikePartNo(string tok)
    {
        var t = (tok ?? "").Trim();
        ...
        foreach (var ch in t)
        {
            ...
            bool isHyphen = ch == '-' || (ch >= '‐' && ch <= '―'); // -, ‐‑‒–—―
            if (!(isDigit || isAlpha || isHyphen)) return false;
            if (isDigit || isAlpha) hasAlnum = true;
        }
        return hasAlnum;
    }
