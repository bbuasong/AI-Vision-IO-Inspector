...
// ── 출력 디렉토리 / Jobs 설정 ────────────────────────────────────────────────
var outputDir = ResolveOutputDir(app.Configuration["ScanOutputDir"]);
var scannerEndpointsEnabled = bool.TryParse(app.Configuration["EnableScannerEndpoints"], out bool enableScannerEndpoints)
    && enableScannerEndpoints;
...
// ── 유틸 ─────────────────────────────────────────────────────────────────────
static string ResolveOutputDir(string? configuredPath)
{
    ...
    throw new InvalidOperationException("ScanOutputDir cannot be created.", lastError);
}
...
// ── /health ──────────────────────────────────────────────────────────────────
static IResult ScannerEndpointDisabled()
{
    return Results.Problem(
        "Scanner control endpoints are disabled. Use /ocr-image with an existing scanned image_path.",
...
app.MapGet("/scanners", () =>
{
    if (!scannerEndpointsEnabled)
        return ScannerEndpointDisabled();
...
// External image OCR for x64 clients. The WPF simulator scans/prepares the image,
// this x86 API process only runs the Epson/Tesseract OCR pipeline.
app.MapPost("/ocr-image", ([FromBody] OcrImageRequest req, OcrEngine ocr) =>
{
    req.Normalize();

    if (string.IsNullOrWhiteSpace(req.ImagePath))
        return Results.BadRequest(new { detail = "image_path is required." });
    if (!File.Exists(req.ImagePath))
        return Results.NotFound(new { detail = "Image file not found.", image_path = req.ImagePath });

    string outPdf = Path.Combine(outputDir, $"external_{Guid.NewGuid():N}.pdf");
    try
    {
        Dictionary<string, object> info = ocr.ImageToSearchablePdf(req.ImagePath, outPdf, req.Lang, req.Engine);
        info.TryGetValue("text", out object? text);
        info.TryGetValue("part_no", out object? partNo);
        info.TryGetValue("engine", out object? engineName);

        return Results.Ok(new
        {
            image_path = req.ImagePath,
            pdf_path = outPdf,
            engine = engineName?.ToString() ?? req.Engine,
            text = text?.ToString() ?? "",
    ...
    {
        return Results.Problem($"OCR/PDF failed: {ex.Message}", statusCode: 500);
    }
}).WithTags("OCR");
...
// ── POST /scan-to-pdf ─────────────────────────────────────────────────────────
app.MapPost("/scan-to-pdf", ([Microsoft.AspNetCore.Mvc.FromBody] ScanToPdfRequest? body, JobRegistry jobs, OcrEngine ocr) =>
{
    var req = body ?? new();
    req.Normalize();
